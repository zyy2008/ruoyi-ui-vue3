...
public class TSoilPathwayController extends BaseController
{
    ...
    /**
     * 导出土壤途径：含水层以上土壤分层信息列表
     */
    @PreAuthorize("@ss.hasPermi('admin:pathway:export')")
    @Log(title = "土壤途径：含水层以上土壤分层信息", businessType = BusinessType.EXPORT)
    @PostMapping("/export")
    public void export(HttpServletResponse response, TSoilPathway tSoilPathway)
    {
        ...
        util.exportExcel(response, list, "土壤途径：含水层以上土壤分层信息数据");
    }

    /**
     * 获取土壤途径：含水层以上土壤分层信息详细信息
     */
    ...
    @Log(title = "土壤途径：含水层以上土壤分层信息", businessType = BusinessType.INSERT)
    @PostMapping
    ...
    @Log(title = "土壤途径：含水层以上土壤分层信息", businessType = BusinessType.UPDATE)
    @PutMapping
    ...
    @Log(title = "土壤途径：含水层以上土壤分层信息", businessType = BusinessType.DELETE)
	@DeleteMapping("/{ids}")
