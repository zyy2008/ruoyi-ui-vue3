<template>
  ...
  <div class="map-tools">
    <div class="tool-button" @click="handleToolClick('layer')" :class="{ active: currentTool === 'layer' }">
      <!-- <el-icon><Position /></el-icon> -->
      <svg-icon icon-class="tcgl" />
      <span>图层管理</span>
    </div>

    <div class="tool-button" @click="handleToolClick('location')" :class="{ active: currentTool === 'location' }">
      <el-icon>
        <Position />
      </el-icon>
      <span>坐标查询</span>
    </div>
    <div class="tool-button" @click="handleToolClick('distance')" :class="{ active: currentTool === 'distance' }">
      <el-icon>
        <Connection />
      </el-icon>
      <span>距离测量</span>
    </div>
    <div class="tool-button" @click="handleToolClick('area')" :class="{ active: currentTool === 'area' }">
      <el-icon>
        <FullScreen />
      </el-icon>
      <span>面积测量</span>
  ...
  <el-dialog modal-class="visible_Distance map-dialog" draggable :title="'距离测量'" v-model="visible_Distance"
    width="350px" :modal="false" :close-on-click-modal="false" :destroy-on-close="true">
    <Distance />
  </el-dialog>

  <el-dialog modal-class="visible_Area map-dialog" draggable :title="'面积测量'" v-model="visible_Area" width="350px"
    :modal="false" :close-on-click-modal="false" :destroy-on-close="true">
    <Area />
  </el-dialog>
</template>
